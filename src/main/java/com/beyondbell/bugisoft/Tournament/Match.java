...
import com.beyondbell.bugisoft.Utilities.RainbowSixSiege.Map;
import com.beyondbell.bugisoft.Utilities.RainbowSixSiege.Operator;
...
class Match {
	...
	public Match(Player[] blueTeam, Player[] orangeTeam, int numberOfMaps, boolean shouldBanOperators, int numberOfOperatorBans, boolean shouldBanOperatorsPerMap) {
		...
		System.arraycopy(blueTeam, 0, players, 0, blueTeam.length);
		System.arraycopy(orangeTeam, blueTeam.length, players, blueTeam.length, blueTeam.length + orangeTeam.length - blueTeam.length);
